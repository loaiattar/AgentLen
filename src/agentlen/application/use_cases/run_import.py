    ...
    async def _import_batch(
        ...
        *,
        ...
        async with self._uow as uow:
            ...
            collisions = [
                *_collisions("model_call", model_calls, model_outcome, normalised, counters),
                *_collisions("tool_call", tool_calls, tool_outcome, normalised, counters),
            ]
...
def _collisions(
    ...
    origins: dict[UUID, tuple[int, str]] | None = None  # built on the first collision
    issues: list[ImportIssue] = []
    for row, _ in calls:
