...
| Accès anonyme à l'API | Middleware `X-API-Key` sur toutes les routes sauf `/health`, `/version` (sans accès à la base) et la documentation OpenAPI (`/docs`, `/redoc`, `/openapi.json`). Clé lue dans `API_KEY`, jamais journalisée. CORS limité à `ALLOWED_ORIGINS`. |
